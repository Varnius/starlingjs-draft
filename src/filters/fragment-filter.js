...
export default class FragmentFilter extends EventDispatcher {
  ...
  render(painter) {
    ...
    if (!_target)
      throw new Error(
        '[IllegalOperationError] Cannot render filter without target'
      )

    if (_target.is3D) this._cached = this._cacheRequested = false
  ...
  renderPasses(painter, forCache) {
    ...
    if (
      !forCache &&
      (this._alwaysDrawToBackBuffer || this._target.requiresRedraw)
    ) {
    ...
    RectangleUtil.extendToWholePixels(
      bounds,
      window.StarlingContextManager.current.contentScaleFactor
    )

    this._helper.textureScale =
      window.StarlingContextManager.current.contentScaleFactor *
      this._resolution
    ...
    if (output) {
      ...
      if (this._target.is3D) painter.state.setModelviewMatricesToIdentity()
      // -> stage coords
  ...
  process(painter, helper, input0 = null) {
    ...
    if (output) {
      // render to texture
      renderTarget = output
      projectionMatrix = MatrixUtil.createPerspectiveProjectionMatrix(
        0,
        0,
        output.root.width / this._resolution,
        output.root.height / this._resolution,
        0,
        0,
        null,
        FragmentFilter.sMatrix3D
      )
    } else {
...
class FilterQuad extends Mesh {
  ...
  moveVertices(sourceSpace, targetSpace) {
    if (targetSpace.is3D) throw new Error('cannot move vertices into 3D space')
    else if (sourceSpace !== targetSpace) {
      targetSpace
        .getTransformationMatrix(sourceSpace, FilterQuad.sMatrix)
        .invert() // ss could be null!
  ...
  set texture(value) {
    super.texture = value
    if (value)
      value.setupTextureCoordinates(this.vertexData, 0, 'texCoords', true)
  }
}
