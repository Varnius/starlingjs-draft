...
export default class IndexData {
  ...
  clear() {
    if (this._rawData) this._rawData = new Uint16Array(this._initialCapacity)
  ...
  copyTo(target, targetIndexID = 0, offset = 0, indexID = 0, numIndices = -1) {
    ...
    if (this._useQuadLayout) {
      if (target._useQuadLayout) {
        ...
        if (
          ...
          for (let i = 0; i < numIndices; ++i)
            keepsQuadLayout =
              keepsQuadLayout &&
              IndexData.getBasicQuadIndexAt(indexID + i) + offset ===
                IndexData.getBasicQuadIndexAt(targetIndexID + i)
        }
      ...
      if (target._useQuadLayout) target.switchToGenericData()
  ...
  setIndex(indexID, index) {
    if (this._numIndices < indexID + 1) this.numIndices = indexID + 1
  ...
  addQuad(a, b, c, d) {
    if (this._useQuadLayout) {
      if (
        a === IndexData.getBasicQuadIndexAt(this._numIndices) &&
        b === a + 1 &&
        c === b + 1 &&
        d === c + 1
      ) {
