...
export default class BlurFilter extends FragmentFilter {
    ...
    updatePadding() {
        const paddingX = this._blurX ? (this.totalBlurX * 3 + 2) / this.resolution : 0;
        const paddingY = this._blurY ? (this.totalBlurY * 3 + 2) / this.resolution : 0;
    ...
    get numPasses() {
        ...
        else return this.getNumPasses(this.totalBlurX) + this.getNumPasses(this.totalBlurY);
    }
...
class BlurEffect extends FilterEffect {
    ...
    updateParameters() {
        ...
        let offset1, offset2;
        const pixelSize = 1.0 / (this._direction === BlurFilter.HORIZONTAL ?
                this.texture.root.nativeWidth : this.texture.root.nativeHeight);
