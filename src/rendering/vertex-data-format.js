...
export default class VertexDataFormat {
  ...
  static fromString(format) {
    ...
    if (format in sFormats) return sFormats[format]
      ...
      if (normalizedFormat in sFormats) instance = sFormats[normalizedFormat]
  ...
  setVertexBufferAt(index, buffer, attrName) {
    ...
    window.StarlingContextManager.current.context.setVertexBufferAt(
      index,
      buffer,
      attribute.offset / 4,
      attribute.format
  ...
  parseFormat(format) {
    ...
    if (!!format && format !== '') {
      ...
      for (let i = 0; i < numParts; ++i) {
        ...
        this._format +=
          (i === 0 ? '' : ', ') + attribute.name + ':' + attribute.format
