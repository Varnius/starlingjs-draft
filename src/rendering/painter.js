...
export default class Painter {
  ...
  constructor(canvas) {
    ...
    if (!gl) {
      console.log(
        'Dafuq, WebGL 2 is not available.  See <a href="https://www.khronos.org/webgl/wiki/Getting_a_WebGL_Implementation">How to get a WebGL 2 implementation</a>'
      )
      return
    }
  ...
  setStateTo(transformationMatrix, alphaFactor = 1.0, blendMode = 'auto') {
    if (transformationMatrix)
      MatrixUtil.prependMatrix(
        this._state._modelviewMatrix,
  ...
  restoreState() {
    if (this._stateStackPos < 0)
      throw new Error(
        '[IllegalOperationError] Cannot restore from empty state stack'
      )
  ...
  pushClipRect(clipRect) {
    ...
    else intersection.copyFrom(clipRect)
  ...
  isRectangularMask(mask, maskee, out) {
    ...
    if (quad && !is3D && !quad.texture && !maskee.maskInverted) {
      ...
      return (
        (MathUtil.isEquivalent(out.a, 0) && MathUtil.isEquivalent(out.d, 0)) ||
  ...
  applyDepthTest() {
    ...
    if (
      depthMask !== this._actualDepthMask ||
      depthTest !== this._actualDepthTest
    ) {
      const gl = this._context
  ...
  applyRenderTarget() {
    ...
    if (
      target !== this._actualRenderTarget ||
      options !== this._actualRenderTargetOptions
    ) {
      if (target) {
  ...
  applyClipRect() {
    ...
    if (clipRect) {
      ...
      gl.scissor(
        sScissorRect.x,
        height - sScissorRect.y - sScissorRect.height,
        sScissorRect.width,
        sScissorRect.height
      )
    } else {
