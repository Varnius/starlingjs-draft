...
export default class BenchmarkScene extends Scene {
  ...
  dispose() {
    ...
    for (const object of this._objectPool) object.dispose()
  ...
  onEnterFrame = (event, passedTime) => {
    ...
    if (this._phase === 0) {
      if (measuredFps < 0.985 * _targetFps) {
        ...
        if (this._failCount === MAX_FAIL_COUNT) this._phase = 1
      } else {
  ...
  removeTestObjects(count) {
    ...
    for (let i = 0; i < count; ++i)
      this.putObjectToPool(
        this._container.removeChildAt(this._container.numChildren - 1)
      )
  }
