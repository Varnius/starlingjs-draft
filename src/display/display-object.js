...
export default class DisplayObject extends EventDispatcher {
  ...
  hitTest(localPoint) {
    ...
    if (getBounds(this, DisplayObject.sHelperRect).containsPoint(localPoint))
      return this
    return null
  }
  ...
  hitTestMask(localPoint) {
    ...
    if (_mask) {
      if (_mask.stage)
        getTransformationMatrix(_mask, DisplayObject.sHelperMatrixAlt)
      else {
      ...
      const helperPoint =
        localPoint === sHelperPoint ? new Point() : sHelperPoint
      MatrixUtil.transformPoint(
        DisplayObject.sHelperMatrixAlt,
        localPoint,
        helperPoint
      )
  ...
  alignPivot(horizontalAlign = 'center', verticalAlign = 'center') {
    ...
    if (horizontalAlign === Align.LEFT) this._pivotX = bounds.x
      ...
      this._pivotX = bounds.x + bounds.width / 2.0
    else if (horizontalAlign === Align.RIGHT)
      this._pivotX = bounds.x + bounds.width
    else
      throw new Error(
        '[ArgumentError] Invalid horizontal alignment: ' + horizontalAlign
      )

    if (verticalAlign === Align.TOP) this._pivotY = bounds.y
    else if (verticalAlign === Align.CENTER)
      this._pivotY = bounds.y + bounds.height / 2.0
    else if (verticalAlign === Align.BOTTOM)
      this._pivotY = bounds.y + bounds.height
    else
      throw new Error(
        '[ArgumentError] Invalid vertical alignment: ' + verticalAlign
      )
  }
  ...
  local3DToGlobal(localPoint, out = null) {
    ...
    if (!stage)
      throw new Error('[IllegalOperationError] Object not connected to stage')
  ...
  setParent(value) {
    ...
    let ancestor = value
    while (ancestor !== this && !!ancestor) ancestor = ancestor._parent

    if (ancestor === this)
      throw new Error(
        '[ArgumentError] An object cannot be added as a child to itself or one ' +
  ...
  /** @private */
  dispatchEvent(event) {
    if (event.type === Event.REMOVED_FROM_STAGE && !this.stage) return // special check to avoid double-dispatch of RfS-event.
  ...
  set transformationMatrix(matrix) {
    ...
    this._scaleY =
      this._skewX > -PI_Q && this._skewX < PI_Q
        ? matrix.d / Math.cos(this._skewX)
        : -matrix.c / Math.sin(this._skewX)
    this._scaleX =
      this._skewY > -PI_Q && this._skewY < PI_Q
        ? matrix.a / Math.cos(this._skewY)
  ...
  get transformationMatrix3D() {
    if (!this._transformationMatrix3D)
      this._transformationMatrix3D = MatrixUtil.convertTo3D(
  ...
  set useHandCursor(value) {
    ...
    else this.removeEventListener(TouchEvent.TOUCH, this.onTouch)
  }
  ...
  get root() {
    ...
    while (currentObject._parent) {
      ...
      if (currentObject._parent.constructor.name === 'Stage')
        return currentObject
