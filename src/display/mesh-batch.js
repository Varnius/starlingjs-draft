...
export default class MeshBatch extends Mesh {
  ...
  addMesh(
    ...
    if (targetVertexID === 0) this.setupFor(mesh)

    meshStyle.batchVertexData(
      this._style,
      targetVertexID,
      matrix,
      subset.vertexID,
    ...
    meshStyle.batchIndexData(
      this._style,
      targetIndexID,
      targetVertexID - subset.vertexID,
      subset.indexID,
      subset.numIndices
    )

    if (alpha !== 1.0)
      this._vertexData.scaleAlphas(
        'color',
        alpha,
        targetVertexID,
        subset.numVertices
  ...
  addMeshAt(mesh, indexID, vertexID) {
    ...
    if (this._vertexData.numVertices === 0) this.setupFor(mesh)
    ...
    if (this.alpha !== 1.0)
      this._vertexData.scaleAlphas('color', this.alpha, vertexID, numVertices)
    if (this._parent) this.setRequiresRedraw()
  ...
  canAddMesh(mesh, numVertices = -1) {
    ...
    if (numVertices + currentNumVertices > MeshBatch.MAX_NUM_VERTICES)
      return false
