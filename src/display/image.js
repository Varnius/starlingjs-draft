...
export default class Image extends Quad {
  ...
  set scaleX(value) {
    super.scaleX = value
    if (this.texture && (this._scale9Grid || this._tileGrid))
      this.setupVertices()
  }
  ...
  setupScale9Grid() {
    ...
    if (MathUtil.isEquivalent(this._scale9Grid.width, texture.frameWidth))
      ...
      MathUtil.isEquivalent(this._scale9Grid.height, texture.frameHeight)
    )
      absScaleX /= absScaleY
    ...
    if (pixelBounds.x < gridCenter.x) sBasCols[0] = gridCenter.x - pixelBounds.x

    if (pixelBounds.y < gridCenter.y) sBasRows[0] = gridCenter.y - pixelBounds.y
    ...
    if (pixelBounds.x < gridCenter.x) sPadding.left = pixelBounds.x * invScaleX
    else sPadding.left = gridCenter.x * invScaleX + pixelBounds.x - gridCenter.x

    if (pixelBounds.right > gridCenter.right)
      sPadding.right = (textureBounds.width - pixelBounds.right) * invScaleX
    else
      sPadding.right =
        (textureBounds.width - gridCenter.right) * invScaleX +
        gridCenter.right -
        pixelBounds.right

    if (pixelBounds.y < gridCenter.y) sPadding.top = pixelBounds.y * invScaleY
    else sPadding.top = gridCenter.y * invScaleY + pixelBounds.y - gridCenter.y

    if (pixelBounds.bottom > gridCenter.bottom)
      sPadding.bottom = (textureBounds.height - pixelBounds.bottom) * invScaleY
    else
      sPadding.bottom =
        (textureBounds.height - gridCenter.bottom) * invScaleY +
        gridCenter.bottom -
        pixelBounds.bottom
    ...
    if (sPosCols[1] <= 0) {
      correction =
        (textureBounds.width / (textureBounds.width - gridCenter.width)) *
        absScaleX
    ...
    if (sPosRows[1] <= 0) {
      correction =
        (textureBounds.height / (textureBounds.height - gridCenter.height)) *
        absScaleY
    ...
    const numVertices = this.setupScale9GridAttributes(
      sPadding.left,
      sPadding.top,
      sPosCols,
      sPosRows,
  ...
  setupTileGrid() {
    ...
    let frameWidth =
      this._tileGrid.width > 0 ? this._tileGrid.width : texture.frameWidth
    let frameHeight =
      this._tileGrid.height > 0 ? this._tileGrid.height : texture.frameHeight
    ...
    while (currentY < bounds.height) {
      ...
      while (currentX < bounds.width) {
        ...
        posRight =
          currentX + tileWidth > bounds.width
            ? bounds.width
            : currentX + tileWidth
        posBottom =
          currentY + tileHeight > bounds.height
            ? bounds.height
            : currentY + tileHeight
  ...
  static automateSetupForTexture(texture, onAssign, onRelease = null) {
    if (!texture) return
    else if (!onAssign && !onRelease) Image.sSetupFunctions.delete(texture)
    else {
