...
export default class Tween extends EventDispatcher {
    ...
    reset(target, time, transition = 'linear') {
        ...
        if (SystemUtil.isString(transition))
            this.transition = transition;
        else if (transition instanceof Function)
            this.transitionFunc = transition;
        else
            throw new Error('[ArgumentError] Transition must be either a string or a function');
        ...
        if (this._updateFuncs) this._updateFuncs.length = 0; else this._updateFuncs = [];
    ...
    /** @inheritDoc */
    advanceTime(time) {
        const { _reverse, _totalTime, _properties, _startValues, _endValues, _onRepeat, _onRepeatArgs, _onComplete,
            _onCompleteArgs, _onUpdateArgs, _repeatDelay, _updateFuncs, _onStart, _transitionFunc, _onUpdate, _target } = this;

        if (time === 0 || (this._repeatCount === 1 && this._currentTime === _totalTime)) return;
        ...
        else if (this._currentTime > _totalTime)
        ...
        this._progress = reversed ? _transitionFunc(1.0 - ratio) : _transitionFunc(ratio);
    ...
    updateRgb = (property, startValue, endValue) => {
        this._target[property] = Color.interpolate(startValue, endValue, this._progress); // todo: was uint
    };
    ...
    getEndValue(property) {
        ...
        if (index === -1) throw new Error(`[ArgumentError] The property '${property}' is not animated`);
