...
export default class BatchProcessor {
  ...
  dispose() {
    for (const batch of this._batches) batch.dispose()
  ...
  addMesh(mesh, state, subset = null, ignoreTransformations = false) {
    ...
    if (subset.numVertices > 0) {
      if (
        !this._currentBatch ||
        !this._currentBatch.canAddMesh(mesh, subset.numVertices)
      ) {
        this.finishBatch()
  ...
  finishBatch() {
    ...
    if (meshBatch) {
      ...
      if (this._onBatchComplete) this._onBatchComplete(meshBatch)
    }
  }
  ...
  clear() {
    ...
    for (let i = 0; i < numBatches; ++i) this._batchPool.put(this._batches[i])
...
class BatchPool {
  ...
  purge() {
    for (const batchList of this._batchLists) {
      for (let i = 0; i < batchList.length; ++i) batchList[i].dispose()
