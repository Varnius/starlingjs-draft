...
export default class MovieClip extends Image {
  ...
  init(textures, fps) {
    ...
    for (let i = 0; i < numFrames; ++i)
      this._frames[i] = new MovieClipFrame(
        textures[i],
        this._defaultFrameDuration,
        this._defaultFrameDuration * i
      )
  }
  ...
  removeFrameAt(frameID) {
    if (frameID < 0 || frameID >= this.numFrames)
      throw new Error('[ArgumentError] Invalid frame id')
    if (this.numFrames === 1)
      throw new Error('[IllegalOperationError] Movie clip must not be empty')

    this._frames.splice(frameID, 1)

    if (frameID !== this.numFrames) this.updateStartTimes()
  }
  ...
  advanceTime(passedTime) {
    ...
    while (passedTime >= restTimeInFrame) {
      ...
      if (changedFrame) frame.playSound(this._soundTransform)
      ...
      if (
        passedTime + 0.0001 > restTimeInFrame &&
        passedTime - 0.0001 < restTimeInFrame
      )
        passedTime = restTimeInFrame
    }
  ...
  set currentTime(value) {
    if (value < 0 || value > this.totalTime)
      throw new Error('[ArgumentError] Invalid time: ' + value)
    ...
    while (
      this._currentFrameID < lastFrameID &&
      _frames[this._currentFrameID + 1].startTime <= value
    )
  ...
  set currentFrame(value) {
    if (value < 0 || value >= this.numFrames)
      throw new Error('[ArgumentError] Invalid frame id')
  ...
  set fps(value) {
    ...
    for (let i = 0; i < this.numFrames; ++i) _frames[i].duration *= acceleration
   ...
   *  is reached. */
  get isPlaying() {
    if (this._playing) return this._loop || this._currentTime < this.totalTime
    else return false
  }
  ...
  executeAction(movie, frameID) {
    if (this.action) {
      ...
      if (numArgs === 0) this.action()
        ...
        throw new Error(
          'Frame actions support zero, one or two parameters: movie:MovieClip, frameID'
        )
    }
  }
}
