...
export default class Mesh extends DisplayObject {
  ...
  constructor(vertexData, indexData, style = null) {
    ...
    if (!vertexData)
      throw new Error('[ArgumentError] VertexData must not be null')
    if (!indexData)
      throw new Error('[ArgumentError] IndexData must not be null')
  ...
  createDefaultMeshStyle() {
    ...
    if (Mesh.sDefaultStyleFactory) {
      if (Mesh.sDefaultStyleFactory.length === 0)
