...
export default class TextField extends DisplayObjectContainer {
  ...
  constructor(width, height, text = '', format = null, options = null) {
    ...
    if (!TextField.sDefaultCompositor)
      TextField.sDefaultCompositor = new TrueTypeCompositor()
  ...
  dispose() {
    this._format.removeEventListener(
      Event.CHANGE,
      this.setRequiresRecomposition
    )
    this._options.removeEventListener(
      Event.CHANGE,
      this.setRequiresRecomposition
    )
  ...
  updateText() {
    ...
    this._options.textureScale =
      window.StarlingContextManager.current.contentScaleFactor
    this._compositor.fillMeshBatch(
      this._meshBatch,
      width,
      height,
      this._text,
    ...
    if (this._customStyle) this._meshBatch.style = this._customStyle
      ...
      this._defaultStyle = this._compositor.getDefaultMeshStyle(
        this._defaultStyle,
        this._format,
        this._options
      )
    ...
    if (this._options.autoSize !== TextFieldAutoSize.NONE) {
      this._textBounds = this._meshBatch.getBounds(
        this._meshBatch,
        this._textBounds
      )
  ...
  get isHorizontalAutoSize() {
    return (
      this._options.autoSize === TextFieldAutoSize.HORIZONTAL ||
  ...
  get isVerticalAutoSize() {
    return (
      this._options.autoSize === TextFieldAutoSize.VERTICAL ||
      this._options.autoSize === TextFieldAutoSize.BOTH_DIRECTIONS
    )
  }
  ...
  set border(value) {
    if (value && !this._border) {
      ...
      for (let i = 0; i < 4; ++i) this._border.addChild(new Quad(1.0, 1.0))
  ...
  static get compositors() {
    let compositors =
      window.StarlingContextManager.current.painter.sharedData[
        TextField.COMPOSITOR_DATA_NAME
      ]
