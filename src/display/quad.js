...
export default class Quad extends Mesh {
  ...
  constructor(width, height, color = 0xffffff) {
    ...
    if (width === 0.0 || height === 0.0)
      throw new Error(
        '[ArgumentError] Invalid size: width and height must not be zero'
      )
  ...
  getBounds = (targetSpace, out = null) => {
    ...
    } else if (targetSpace === this.parent && !isRotated) {
      // optimization
  ...
  readjustSize = (width = -1, height = -1) => {
    ...
    if (height <= 0)
      height = texture ? texture.frameHeight : this._bounds.height
