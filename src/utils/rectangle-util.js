...
export default class RectangleUtil {
  ...
  static intersect(rect1, rect2, out = null) {
    ...
    if (left > right || top > bottom) out.setEmpty()
    else out.setTo(left, top, right - left, bottom - top)
  ...
  static fit(
    ...
    if (!ScaleMode.isValid(scaleMode))
      throw new Error('[ArgumentError] Invalid scaleMode: ' + scaleMode)
    if (!out) out = new Rectangle()
    ...
    for (let i = 0; i < 4; ++i) {
      ...
      if (matrix)
        MatrixUtil.transformCoords3D(
          ...
          position.y,
          0,
          RectangleUtil.RectangleUtil.sPoint3D
        )
      else RectangleUtil.RectangleUtil.sPoint3D.setTo(position.x, position.y, 0)
  ...
  static getPositions(rectangle, out = null) {
    ...
    for (let i = 0; i < 4; ++i) if (!out[i]) out[i] = new Point()
