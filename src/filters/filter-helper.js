...
export default class FilterHelper {
  ...
  getTexture(resolution = 1.0) {
    ...
    if (this._numPasses >= 0) if (this._numPasses-- === 0) return null
    ...
    if (
      !MathUtil.isEquivalent(texture.width, this._width, 0.1) ||
      !MathUtil.isEquivalent(texture.height, this._height, 0.1) ||
      !MathUtil.isEquivalent(texture.scale, this._scale * resolution)
    ) {
  ...
  putTexture(texture) {
    if (texture) {
      if (
        texture.root.nativeWidth === this._nativeWidth &&
        texture.root.nativeHeight === this._nativeHeight
      )
  ...
  getNativeSize(size, textureScale) {
    ...
    if (this._usePotTextures)
      return nativeSize > this._sizeStep
        ? MathUtil.getNextPowerOfTwo(nativeSize)
        : this._sizeStep
    else return Math.ceil(nativeSize / this._sizeStep) * this._sizeStep
  }
