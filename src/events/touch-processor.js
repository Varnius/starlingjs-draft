...
export default class TouchProcessor {
  ...
  advanceTime(passedTime) {
    ...
    while (_queue.length > 0 || numIterations === 0) {
      ...
      while (
        _queue.length > 0 &&
        !this.containsTouchWithID(sUpdatedTouches, _queue[_queue.length - 1][0])
      ) {
      ...
      for (i = _currentTouches.length - 1; i >= 0; --i) {
        ...
        if (
          ...
          !this.containsTouchWithID(sUpdatedTouches, touch.id)
        ) {
  ...
  enqueue(...args) {
    ...
    if (this._ctrlDown && this._touchMarker && touchID === 0) {
      ...
      this._queue.unshift([
        1,
        phase,
        this._touchMarker.mockX,
        this._touchMarker.mockY
      ])
    }
  }
  ...
  enqueueMouseLeftStage() {
    ...
    else if (minDist === distRight) exitX = this._stage.stageWidth + offset
    else if (minDist === distTop) exitY = -offset
    else exitY = this._stage.stageHeight + offset
  ...
  createOrUpdateTouch(
    ...
    if (phase === TouchPhase.BEGAN) this.updateTapCount(touch)
  ...
  updateTapCount(touch) {
    ...
    for (const tap of _lastTaps) {
      const sqDist =
        Math.pow(tap.globalX - touch.globalX, 2) +
  ...
  addCurrentTouch(touch) {
    ...
    for (let i = _currentTouches.length - 1; i >= 0; --i)
      if (_currentTouches[i].id === touch.id) _currentTouches.splice(i, 1)
  ...
  containsTouchWithID(touches, touchID) {
    for (const touch of touches) if (touch.id === touchID) return true
  ...
  set simulateMultitouch(value) {
    ...
    if (value && !this._touchMarker) {
      if (window.StarlingContextManager.current.contextValid)
        createTouchMarker()
      else target.addEventListener(Event.CONTEXT3D_CREATE, createTouchMarker)
    ...
    if (event.key === 'z') {
      ...
      if (_touchMarker && wasCtrlDown !== this._ctrlDown) {
        ...
        if (
          wasCtrlDown &&
          mockedTouch &&
          mockedTouch.phase !== TouchPhase.ENDED
        ) {
          // end active touch ...
          _queue.unshift([
            1,
          ...
          // ... or start new one
          if (
            mouseTouch.phase === TouchPhase.HOVER ||
