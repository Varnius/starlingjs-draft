...
export default class VertexData {
  ...
  constructor(format = null, initialCapacity = 32, createRawData = true) {
    ...
    else if (typeof format === 'string' || format instanceof String)
      this._format = VertexDataFormat.fromString(format)
    else
      throw new Error(
        "[ArgumentError] 'format' must be String or VertexDataFormat"
      )

    this._attributes = this._format.attributes
    this._numAttributes = this._attributes.length
    this._posOffset = this._format.hasAttribute('position')
      ? this._format.getOffset('position')
      : 0
    this._colOffset = this._format.hasAttribute('color')
      ? this._format.getOffset('color')
      : 0
    ...
    if (_format === target._format) {
      ...
      if (matrix) {
        ...
        while (pos < endPos) {
          ...
          target._rawData.setFloat32(
            pos + 4,
            matrix.d * y + matrix.b * x + matrix.ty,
            true
          )
    ...
    if (matrix) {
      ...
      for (i = 0; i < numVertices; ++i) {
        for (j = 0; j < attributeSizeIn32Bits; ++j) {
          targetData.setUint32(
            targetPos,
            sourceData.getUint32(sourcePos, true),
            true
          )
          sourcePos += 4
          targetPos += 4
  ...
  toString() {
    return `[VertexData format="${this._format.formatString} numVertices=${
      this._numVertices
    }]`
  }
  ...
  getColor(vertexID, attrName = 'color') {
    ...
    const offset =
      attrName === 'color' ? _colOffset : this.getAttribute(attrName).offset
    const position = vertexID * _vertexSize + offset
  ...
  getBounds(
    ...
    if (numVertices === 0) {
      ...
      const offset =
        attrName === 'position'
          ? _posOffset
          : this.getAttribute(attrName).offset
  ...
  getBoundsProjected(
    ...
    if (numVertices === 0) {
      if (matrix) MatrixUtil.transformCoords3D(matrix, 0, 0, 0, sHelperPoint3D)
      else sHelperPoint3D.setTo(0, 0, 0)
      ...
      for (i = 0; i < numVertices; ++i) {
        ...
        else sHelperPoint3D.setTo(x, y, 0)
      ...
      for (let i = 0; i < _numAttributes; ++i) {
        ...
        if (attribute.isColor) {
          ...
          for (let j = 0; j < _numVertices; ++j) {
            ...
            newColor = value
              ? premultiplyAlpha(oldColor)
              : unmultiplyAlpha(oldColor)
  ...
  scaleAlphas(attrName, factor, vertexID = 0, numVertices = -1) {
    ...
    for (let i = 0; i < numVertices; ++i) {
      ...
      if (alpha === 1.0 || !_premultipliedAlpha) {
        _rawData.setUint32(
          colorPos,
          Color.setAlphaRgba(color, alpha * 255.0),
          true
        )
      } else {
  ...
  colorize(
    ...
    if (rgba === 0xffffffff && numVertices === this._numVertices)
      this._tinted = false
