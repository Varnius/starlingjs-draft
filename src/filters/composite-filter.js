...
class CompositeEffect extends FilterEffect {
  static VERTEX_FORMAT = FilterEffect.VERTEX_FORMAT.extend(
  ...
  constructor(numLayers = 4) {
    ...
    if (numLayers < 1 || numLayers > 4)
      throw new Error(
        '[Argument error] number of layers must be between 1 and 4'
      )

    this._layers = []

    for (let i = 0; i < numLayers; ++i) this._layers[i] = new CompositeLayer()
  }
  ...
  getUsedLayers(out = null) {
    ...
    for (const layer of this._layers) if (layer.texture) out[out.length] = layer
    ...
    if (layers.length) {
      const vertexShader = `#version 300 es
                ...
                ${layers
                  .map(
                    (layer, i) =>
                      `layout(location = ${i + 1}) in vec2 aLayerTexCoords${i +
                        1};
                ...
                void main() {
                    ...
                    ${layers
                      .map(
                        (layer, i) =>
                          `vLayerTexCoords${i} = aLayerTexCoords${i +
                            1} + uLayerOffset${i}.xy;`
                      )
                      .join('\n')}
                }
      ...
      const fragmentShader = `#version 300 es
                ...
                void main() {
                    ...
                    ${
                      i !== 0
                        ? `result = (ones - layer${i}.wwww) * layer0 + layer${i};`
                        : ''
                    }
                    `
                      )
  ...
  get programVariantName() {
    ...
    for (let i = 0; i < numLayers; ++i) {
      layer = layers[i]
      bits =
        RenderUtil.getTextureVariantBits(layer.texture) |
        (Math.floor(layer.replaceColor) << 3) // todo: was int()
      totalBits |= bits << (i * 4)
    }
  ...
  beforeDraw(context) {
    ...
    if (numLayers) {
      for (let i = 0; i < numLayers; ++i) {
        ...
        const offsetLoc = gl.getUniformLocation(
          nativeProgram,
          `uLayerOffset${i}`
        )
