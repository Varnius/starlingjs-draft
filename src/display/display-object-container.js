...
export default class DisplayObjectContainer extends DisplayObject {
  ...
  getChildAt(index) {
    ...
    if (index >= 0 && index < numChildren) return this._children[index]
  ...
  setChildIndex(child, index) {
    ...
    if (oldIndex === -1)
      throw new Error('[ArgumentError] Not a child of this container')
  ...
  sortChildren(compareFunction) {
    ...
    DisplayObjectContainer.mergeSort(
      this._children,
      compareFunction,
      0,
      this._children.length,
      DisplayObjectContainer.sSortBuffer
    )
  ...
  getBounds(targetSpace, out = null) {
    ...
    if (numChildren === 0) {
      this.getTransformationMatrix(
        targetSpace,
        DisplayObjectContainer.sHelperMatrix
      )
      MatrixUtil.transformCoords(
        DisplayObjectContainer.sHelperMatrix,
        0.0,
        0.0,
        DisplayObjectContainer.sHelperPoint
      )
      out.setTo(
        DisplayObjectContainer.sHelperPoint.x,
        DisplayObjectContainer.sHelperPoint.y,
        0,
        0
      )
  ...
  render(painter) {
    ...
    for (let i = 0; i < numChildren; ++i) {
      ...
      if (child._hasVisibleArea) {
        if (i !== 0) painter.restoreState()

        if (selfOrParentChanged) child._lastParentOrSelfChangeFrameID = frameID
        ...
        if (cacheEnabled) child._tokenFrameID = frameID
      }
    }
  ...
  broadcastEvent(event) {
    if (event.bubbles)
      throw new Error(
        '[ArgumentError] Broadcast of bubbling events is prohibited'
      )
    ...
    this.getChildEventListeners(
      this,
      event.type,
      DisplayObjectContainer.sBroadcastListeners
    )
  ...
  static mergeSort(input, compareFunc, startIndex, length, buffer) {
    ...
    if (length > 1) {
      ...
      for (i = startIndex; i < endIndex; i++) input[i] = buffer[i - startIndex] // todo: dropped cast to int
    }
  }
  ...
  getChildEventListeners = (object, eventType, listeners) => {
    ...
    if (object.hasEventListener(eventType)) listeners[listeners.length] = object // avoiding 'push'
