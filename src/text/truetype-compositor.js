...
export default class TrueTypeCompositor {
  ...
  constructor() {
    if (!TrueTypeCompositor.sHelperQuad)
      TrueTypeCompositor.sHelperQuad = new Quad(100, 100)
  }
  ...
  fillMeshBatch(meshBatch, width, height, text, format, options = null) {
    ...
    if (format.horizontalAlign === Align.LEFT) sHelperQuad.x = 0
      ...
      sHelperQuad.x = Math.floor((width - texture.width) / 2)
    ...
    if (format.verticalAlign === Align.TOP) sHelperQuad.y = 0
      ...
      sHelperQuad.y = Math.floor((height - texture.height) / 2)
  ...
  renderText(width, height, text, format, options) {
    ...
    ctx.clearRect(
      0,
      0,
      window.StarlingContextManager.current.textCanvas.width,
      window.StarlingContextManager.current.textCanvas.height
    )
    sLines.splice(0)
