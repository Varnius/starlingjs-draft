...
export default class StatsDisplay extends Sprite {
    ...
    constructor() {
        ...
        if (this._background.style.type !== MeshStyle) this._background.style = new MeshStyle();
        if (this._labels.style.type !== MeshStyle) this._labels.style = new MeshStyle();
        if (this._values.style.type !== MeshStyle) this._values.style = new MeshStyle();
    ...
    update() {
        this._background.color = this._skipCount > this._frameCount / 2 ? 0x003F00 : 0x0;
        this._fps = this._totalTime > 0 ? this._frameCount / this._totalTime : 0;
        this._memory = window.performance ? window.performance.memory.usedJSHeapSize * StatsDisplay.B_TO_MB : '';

        const fpsText = this._fps.toFixed(this._fps < 100 ? 1 : 0);
        const memText = this._memory.toFixed(this._memory < 100 ? 1 : 0);
        const drwText = (this._totalTime > 0 ? this._drawCount - 2 : this._drawCount).toString(); // ignore self
